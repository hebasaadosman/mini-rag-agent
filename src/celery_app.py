...
async def get_setup_utils():
    ...
    vectordb_provider_factory = VectorDBProviderFactory(config=settings.dict(), db_client=db_client)

    # generation client
    generation_client = llm_provider_factory.create_provider(provider_name=settings.GENERATION_BACKEND)
    generation_client.set_generation_model(model_id = settings.GENEERATION_MODEL_ID)
                                                               

    # embedding client
    embedding_client = llm_provider_factory.create_provider(provider_name=settings.EMBEDDING_BACKEND)
    embedding_client.set_embedding_model(model_id=settings.EMBEDDING_MODEL_ID,
                                             model_size=settings.EMBEDDING_MODEL_SIZE)
    
    # vector db client
    vectordb_client = vectordb_provider_factory.create_provider(
        provider_name=settings.VECTOR_DB_BACKEND,
        db_path=settings.VECTOR_DB_PATH,
        distance_metric_method=settings.VECTOR_DB_DISTANCE_METRIC_METHOD
    )
...
celery_app.conf.update(
    ...
    task_routes={
        ...
        "tasks.index_processing.index_project": {
